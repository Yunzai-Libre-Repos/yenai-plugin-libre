...
Bot.on?.("message", async(e) => {
  ...
  if (e.message_type == "group") {
    ...
    if (Config.getGroup(e.group_id).groupRecall) {
      // logger.debug(`[Yenai-Plugin-Libre]存储群消息${e.group_id}=>${e.message_id}`)
      // 写入
      await redis.set(
    ...
    if (Config.whole.PrivateRecall) {
      // logger.debug(`[Yenai-Plugin-Libre]存储私聊消息${e.user_id}=>${e.message_id}`)
      // 写入
      await redis.set(
  ...
  if (
    ...
    logger.info("[Yenai-Plugin-Libre]群聊闪照")
    msg = [
    ...
    logger.info("[Yenai-Plugin-Libre]讨论组闪照")
    msg = [
    ...
    logger.info("[Yenai-Plugin-Libre]好友闪照")
    msg = [
      segment.image(`https://q1.qlogo.cn/g?b=qq&s=100&nk=${e.user_id}`),
      `[消息(${e.self_id}) - 闪照消息]\n`,
      `发送人账号：${e.user_id}\n`,
    ...
    logger.info("[Yenai-Plugin-Libre]好友消息")
    msg = [
      segment.image(`https://q1.qlogo.cn/g?b=qq&s=100&nk=${e.user_id}`),
      `[消息(${e.self_id}) - 好友消息]\n`,
      `好友账号：${e.user_id}\n`,
    ...
    logger.info("[Yenai-Plugin-Libre]群临时消息")
    // 发送的消息
    msg = [
      segment.image(`https://q1.qlogo.cn/g?b=qq&s=100&nk=${e.user_id}`),
      `[消息(${e.self_id}) - 群临时消息]\n`,
    ...
    logger.info("[Yenai-Plugin-Libre]群聊消息")
    msg = [
      segment.image(`https://p.qlogo.cn/gh/${e.group_id}/${e.group_id}/100`),
      `[消息(${e.self_id}) - 群聊消息]\n`,
      `来源群号：${e.group_id}\n`,
    ...
    if (!Config.getGroup(e.group_id).groupMessage) return false
    logger.info("[Yenai-Plugin-Libre]讨论组消息")
    msg = [
      segment.image(`https://q1.qlogo.cn/g?b=qq&s=100&nk=${e.user_id}`),
      `[消息(${e.self_id}) - 群聊消息]\n`,
      `来源讨论组号：${e.discuss_id}\n`,
