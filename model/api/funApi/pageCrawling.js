...
export async function pandadiu(type = "cos", keywords = "") {
  ...
  logger.debug("[Yenai-Plugin-Libre][acg]作品索引页：" + homeUrl)
  ...
  logger.debug("[Yenai-Plugin-Libre][acg]图片详情页：" + domain + href)
...
export async function coser() {
  ...
  logger.debug("[Yenai-Plugin-Libre][coser]作品索引页：" + homeUrl)
  const home = await request.get(homeUrl).then(res => res.text())
  const $ = cheerio.load(home)
  const href = _.sample(
    _.map(
  ...
  logger.debug("[Yenai-Plugin-Libre][coser]图片详情页：" + domain + href)
